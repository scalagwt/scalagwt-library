...
object Platform {
  ...
  def createArray(elemClass: Class[_], length: Int): AnyRef =
    sys.error("TODO: Figure out array handling in GWT")

  @inline
  def arrayclear(arr: Array[Int]) { java.util.Arrays.fill(arr, 0) }

  @inline
  def getClassForName(name: String): Class[_] = sys.error("not supported by GWT")
  ...
  @inline
  def defaultCharsetName: String = sys.error("figure this out for gwt")
}
