...
final class StringBuilder(private val underlying: JavaStringBuilder)
  ...
  override def reverse: StringBuilder = sys.error("GWT doesn't support StringBuilder.reverse")
  
  override def clone(): StringBuilder = { sys.error("GWT doesn't support clone method.") }
  ...
  def reverseContents(): StringBuilder = {
    sys.error("GWT doesn't support StringBuilder.reverse")
  }
